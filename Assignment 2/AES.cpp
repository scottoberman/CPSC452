...
bool AES::setKey(const unsigned char* keyArray)
{
	...
	int i, setKeyResult;
	i = setKeyResult = 0;
	unsigned char keyArrayCopy[16];
	memcpy(keyArrayCopy, keyArray + 1, 16);
	...
	if (keyArray[0] == '0')
	{
		setKeyResult = AES_set_encrypt_key(keyArrayCopy, 128, key);
                return true;
	}
	else
	{
		setKeyResult = AES_set_decrypt_key(keyArrayCopy, 128, key);
                return true;
	}
